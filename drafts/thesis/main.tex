...
\begin{comment}
    generalize the definition of $\varepsilon(\mathbf{q}, \omega)$ such that it doesn't depend on the translational invariance of the system. Consider the following definition:
    \begin{equation*}
        \varepsilon(\mathbf{q},\omega) := \frac{1}{(2\pi)^3} \int\!\!\text{d}^3r \!\!\!\int\!\!\text{d}^3r'\; \varepsilon(\mathbf{r}, \mathbf{r'}, \omega)\, e^{-i\mathbf{q}\cdot(\mathbf{r} - \mathbf{r'})}\; .
    \end{equation*}
    In terms of operators, this is just $\langle\mathbf{q} |\hat\varepsilon(\omega)| \mathbf{q}\rangle$
\end{comment}
...
\subsection{Graphene nanotriangles}
    \begin{wrapfigure}[18]{r}{.52\textwidth}
        \center
        \vspace{-1.5cm}
    ...
    \begin{minipage}{\textwidth}
        ...
        \captionof{figure}{Low-energy loss spectrum for a zigzag-edged graphene nanotriangle.}
...
\subsection{Third iteration Sierpinski carpet}
    ...
    As an example, fig. \ref{fig:eigenstate-3rd-SC-0.466250} and \ref{fig:eigenstate-3rd-SC-real-0.474250} show $\operatorname{Re} [\langle \mathbf{r} | \phi_{n(\omega)}(\omega) \rangle]$ for two high peaks at around $0.467$ eV and $0.475$ eV.
    \begin{figure}[h] 
    \begin{minipage}{.5\textwidth}
